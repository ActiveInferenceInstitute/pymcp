...
class PyMDPToolkit:
    ...
    async def run_simulation(self, agent_id, env_id, num_steps=10, save_history=True, planning_horizon=None):
        ...
        response = await self.client.call_tool("run_simulation", params)
        return response.get("result", {})
...
async def run_gridworld_tutorial(client, toolkit, output_dir):
    ...
    simulation_result = await toolkit.run_simulation(
        agent_id=agent_id,
        env_id=env_id,
        num_steps=10,
        save_history=True
...
async def run_examples(host="localhost", port=8080, use_ssl=False, output_dir=None):
    ...
    if not output_dir:
        ...
        output_dir = os.path.join(os.path.dirname(__file__), "output", f"pymcp_examples_{timestamp}")
    ...
    try:
        ...
        # Create client using the MCP protocol standard
        async with MCPClient(server_url=server_url) as client:
            ...
            try:
                ...
                agent_id, env_id = await run_gridworld_tutorial(client, toolkit, output_dir)
                results["gridworld_tutorial"] = {"success": True, "agent_id": agent_id, "env_id": env_id}
            except Exception as e:
            ...
            try:
                ...
                results["free_energy_calculation"] = {
                    "success": True, 
                    "free_energy": fe_results.get("free_energy_components", {}).get("free_energy")
                }
            except Exception as e:
